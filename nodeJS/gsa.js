...
function derivadaGELU(x){
   const cdf=0.5*(1+Math.tanh(Math.sqrt(2/Math.PI)*(x+0.044715*x*x*x)));
   return cdf+x*(1-cdf*cdf)*(0.5*Math.sqrt(2/Math.PI)*(1+3*0.044715*x*x));
}
...
class CamadaAtencao {
  ...
  propagar(x, mascara=null, treino=true) {
    ...
    eNaN(this.bq, "bq", "CamadaAtencao");
    eNaN(this.pk, "pk", "CamadaAtencao");
    eNaN(this.bk, "bk", "CamadaAtencao");
    eNaN(this.pv, "pv", "CamadaAtencao");
    eNaN(this.bv, "bv", "CamadaAtencao");
    eNaN(this.ps, "ps", "CamadaAtencao");
    eNaN(this.bs, "bs", "CamadaAtencao");
...
class CamadaDensa {
  constructor(dimModelo, dimDensa, taxaDropout=0.1) {
    this.p1 = iniPesosHe(dimDensa, dimModelo);
    this.b1 = zeros(dimDensa);
    this.p2 = iniPesosHe(dimModelo, dimDensa);
    this.b2 = zeros(dimModelo);
    // buffers adam:
    this.m1 = matrizZeros(dimDensa, dimModelo);
    this.v1 = matrizZeros(dimDensa, dimModelo);
    this.m2 = matrizZeros(dimModelo, dimDensa);
    this.v2 = matrizZeros(dimModelo, dimDensa);
    this.mB1 = zeros(dimDensa);
    this.vB1 = zeros(dimDensa);
  ...
  retropropagar(dY, taxa, lambda=0.001) {
    ...
    const lote = x.length;
    const dimDensa = this.p1.length;
    const dimEntrada = this.p1[0].length;
    const dimSaida = this.p2.length;
    const dP2 = matrizZeros(dimSaida, dimDensa);
    const dB2 = zeros(dimSaida);
    const dP1 = matrizZeros(dimDensa, dimEntrada);
    const dB1 = zeros(dimDensa);
    ...
    for(let n=0; n<lote; n++) {
      ...
      for(let j=0; j < dimSaida; j++) {
        const err = seqDY[j];
        dB2[j] += err;
        for(let k=0; k<dimDensa; k++) dP2[j][k] += seqo1[k]*err;
      }
      ...
      for(let k=0; k<dimDensa; k++) {
        const deriv = derivadaGELU(seqo1[k]);
        dO[k] = dO[k]*deriv;
      }
      for(let j=0; j<dimDensa; j++) {
        const err1 = dO[j];
        dB1[j] += err1;
...
class BlocoTransformer {
  constructor(dimModelo, numCabecas, dimDensa, taxaDrop) {
    this.atencao = new CamadaAtencao(dimModelo, numCabecas, taxaDrop);
    thia.densa = new CamadaDensa(dimModelo, dimDensa, taxaDrop);
  ...
  propagar(x, mascara=null, treino=true) {
    ...
    const f = thia.densa.propagar(n1, treino);
  ...
  retropropagar(dY, taxa) {
    ...
    const dF = dNorm2;
    const dN1_daF = thia.densa.retropropagar(dF, taxa);
    const dR1_residual = dNorm2;
    const dN1 = dR1_residual.map((v,i)=>somarVetores(v, dN1_daF[i]));
    const dR1 = this.norm1.retropropagar(dN1, taxa);
    const dX_residual = dR1;
    const dA = dR1;
    const dX_daAtencao = this.atencao.retropropagar(dA, taxa);
    const res = dX_residual.map((v,i)=>somarVetores(v, dX_daAtencao[i]));
...
class GSA {
  constructor(config) {
    ...
    this.camadas = [];
    for(let i=0; i<config.numCamadas; i++) this.camadas.push(new BlocoTransformer(config.dimModelo, config.numCabecas, config.dimDensa, this.taxaDropout));
...
class Treinador {
  ...
  treinar(dados, epocas=10, tamanhoLote=8) {
    ...
    for(let epoca=0; epoca<epocas; epoca++) {
      ...
      if(epoca>0 && this.historico.perdas.length>1) {
        let perdaAntiga = this.historico.perdas[epoca-1];
...
function criarGSA(textoTreinamento, config={}) {
  ...
  let merges = [];
  if(fs.existsSync("margesBPE.gsa")) {
    merges = JSON.parse(fs.readFileSync("margesBPE.gsa").toString('utf8'));
    console.log("Merges carregadas de: margesBPE.gsa");
    tokenizador = config.tokenizador || new TokenizadorBPE(merges);
  } else {
    console.log("Treinando tokenizador...");
    const treinoBPE = new TreinadorBPE(1000, 0.1);
...
function obterParams(gsa) {
  ...
  // Densa: p1, p2 + b1, b2
  +gsa.modelo.dimDensa*gsa.modelo.dimModelo
  +gsa.modelo.dimModelo*gsa.modelo.dimDensa
  +gsa.modelo.dimDensa
...
function treinarNovo() {
  ...
  console.log(`  Dimensão Densa: ${gsa.modelo.dimDensa}`);
...
function carregar(caminhoArquivo, config) {
    ...
    const [versao, vocabTam, dimModelo, numCamadas, numCabecas, dimDensa, seqMaxima] = leitor;
    if(versao != SALVAMENTO_VERSAO)throw new Error(`Versão incompatível: arquivo v${versao} | esperado v${SALVAMENTO_VERSAO}`);
    const configCompativel = (config.dimModelo==dimModelo&&config.numCamadas==numCamadas&&config.numCabecas==numCabecas&&config.dimDensa==dimDensa);
    if(!configCompativel) {
    ...
    for(let i=0; i<numCamadas; i++) {
        ...
        // Densa
        carregarMatriz(bloco.ffn.p1, floatBuffer, pos); pos += dimDensa*dimModelo;
        carregarMatriz(bloco.ffn.p2, floatBuffer, pos); pos += dimModelo*dimDensa;
        carregarVetor(bloco.ffn.b1, floatBuffer, pos); pos += dimDensa;
...
function carregarVetor(v, buffer, ini) {
  let pos = ini;
  for(let i=0;i<v.length;i++)v[i]=buffer[pos++];
}
