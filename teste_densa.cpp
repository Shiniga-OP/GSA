...
static void checar(const char* nome, int cond) {
    _total++;
...
void testePropLinear() {
    ...
    checar("saida[0] == 3.5", proximos(saida[0], 3.5f));
    checar("saida[1] == 6.5", proximos(saida[1], 6.5f));
}
...
void testePropReLU() {
    ...
    // pre[0] = 2-1 = 1 -> relu = 1
    // pre[1] = -2+1 = -1 -> relu = 0
    checar("saida[0] == 1 (relu positivo)", proximos(saida[0], 1.0f));
    checar("saida[1] == 0 (relu negativo zerado)", proximos(saida[1], 0.0f));
}
...
void testePropSigmoid() {
    ...
    d.prop(entrada, saida);
    // sigmoid(0) = 0.5
    checar("sigmoid(0) == 0.5", proximos(saida[0], 0.5f));
}
...
void testeGradienteAnaliticoVsNumerico() {
    ...
    checar("erro gradiente entrada < 1%",  errEnt  < 0.01f);
    checar("erro gradiente pesos  < 1%",  errPeso < 0.01f);
...
void testeGradienteMultiplasAtivacoes() {
    ...
    for(int a = 0; a < 7; a++) {
        ...
        checar(nome, err < 0.01f);
    }
}

void testeAcumulacaoGradientelote() {
    printf("\n[6] Acumulacao de gradiente em lote\n");
    srand(1);
    Densa d(2, 2, "relu");
    ...
    checar("gradP acumulado[0,0] == 1", proximos(d.gradP[0], 1.0f));
    checar("gradP acumulado[0,1] == 1", proximos(d.gradP[1], 1.0f));
    checar("zerarGrad funciona", [&]{
        d.zerarGrad();
...
void testeSGDConverge() {
    ...
    checar("perda final < 0.01", perdaFinal < 0.01f);
}
...
void testeEntradaZerada() {
    ...
    checar("saida toda zero com entrada e bias zeros", proximos(y[0],0) && proximos(y[1],0) && proximos(y[2],0));
}
...
void testeEntradaGrande() {
    ...
    checar("sigmoid(100) aprox 1", proximos(yG[0], 1.0f, 1e-3f));
    checar("sigmoid(-100) aprox 0", proximos(yP[0], 0.0f, 1e-3f));
    checar("saidas finitas", isfinite(yG[0]) && isfinite(yP[0]));
}
...
void testeNaNPropagacao() {
    ...
    d.prop(x, y);
    checar("Propagação sem NaN/Inf", tudoFinito(y, 8));

    d.zerarGrad();
    d.retroprop(dLdy, ge);
    checar("gradEntrada sem NaN/Inf", tudoFinito(ge, 8));
    checar("gradPesos sem NaN/Inf", tudoFinito(d.gradP, 8*8));
}

void testeClipGrad() {
    printf("\n[11] clip de gradiente\n");
    Densa d(2, 2, "relu");
    ...
    checar("norma apos clip <= 1.0 + eps", norma <= 1.0f + 1e-4f);
}
...
void testeInicializacaoHe() {
    ...
    checar("variancia proxima de 2/nEnt (±50%%)", var > 0.01f && var < 0.03f);
}

void testeNumParams() {
    printf("\n[13] numParams correto\n");
    Densa d(5, 3, "relu");
    checar("numParams == 5*3 + 3 == 18", d.numParams() == 18);
}
...
void testeCamadaGrande() {
    ...
    checar("Propagação 256->256 finito", tudoFinito(y, 256));
    checar("gradEntrada 256->256 finito", tudoFinito(ge, 256));
    checar("gradPesos 256->256 finito", tudoFinito(d.gradP, 256*256));
...
void testeAtivacaoNula() {
    ...
    float x[2]={1,1}, y[2];
    d.prop(x, y);
    checar("saida[0] == 2+0+1 == 3", proximos(y[0], 3.0f));
    checar("saida[1] == 0+3-1 == 2", proximos(y[1], 2.0f));
}
